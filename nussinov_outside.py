...
nussinov_outside_dp = nussinov_outside_dp.tolist()
for i in range(1,sampleRNA_size):
    for j in range(i):
        nussinov_outside_dp[i][j] = "-"

df = pd.DataFrame(nussinov_outside_dp)

df.to_csv("/Users/futo/Desktop/浅井先生実験/kadai2_output.csv", mode="w")
